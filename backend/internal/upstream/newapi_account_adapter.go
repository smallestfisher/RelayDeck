...
func (a *NewAPIAccountAdapter) TestCall(ctx context.Context, account domain.UpstreamAccount, apiKey string, modelName string, protocol string, streaming bool, message string) (domain.UpstreamTestCallResult, error) {
	start := time.Now()
	var path string
	...
	default:
		return domain.UpstreamTestCallResult{}, fmt.Errorf("unsupported protocol: %s", protocol)
	}
